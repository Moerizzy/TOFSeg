...
Forest = np.array([255, 0, 0])  # label 1
...
def parse_args():
    ...
    parser.add_argument("--img-dir", default="data/tof/train_images")
    parser.add_argument("--mask-dir", default="data/tof/train_masks")
    parser.add_argument("--output-img-dir", default="data/tof/train/images_1024")
    parser.add_argument("--output-mask-dir", default="data/tof/train/masks_1024")
...
def image_augment(image, mask, patch_size, mode="train", val_scale=1.0):
    ...
    if mode == "train":
        ...
        for i in range(len(image_list_train)):
            ...
            # mask_tmp = rgb_to_2D_label(mask_tmp.copy()) # 2D label not needed as it is already 2D
        ...
        # mask = rgb_to_2D_label(mask.copy()) # 2D label not needed as it is already 2D
...
def tof_format(inp):
    ...
    img_filename = "_".join(
        os.path.splitext(os.path.basename(img_path))[0].split("_")[1:]
    )
    mask_filename = "_".join(
        os.path.splitext(os.path.basename(mask_path))[0].split("_")[1:]
    )
    if eroded:
    ...
    for m in range(len(image_list)):
        ...
        for y in range(0, img.shape[0], stride):
            for x in range(0, img.shape[1], stride):
                ...
                print(mask_tile)
                print(mask_tile[np.all(mask_tile == [6, 0, 0], axis=-1)])
                mask_tile[np.all(mask_tile == [6, 0, 0], axis=-1)] = [0, 0, 0]
...
if __name__ == "__main__":
    ...
    t0 = time.time()
    mpp.Pool(processes=mp.cpu_count()).map(tof_format, inp)
    t1 = time.time()
    split_time = t1 - t0
